...
Why a local 3b model and no API: I had no API keys available, and I wanted every number in this report to be reproducible by a reviewer on their own machine with nothing but `ollama pull`. The price is real: a 3b model is the main quality ceiling of the whole system. The drafter paraphrases well when retrieval is good and invents steps when it is not; the judge barely discriminates. Section 9 is mostly about that.
...
**Why SpotifyCares.** I profiled the 15 largest brands in the dump (`scripts/explore_brands.py`). Most are useless for an *answering* agent: the telcos push 72–86% of first replies to "DM us your account", AmazonHelp's 0.6% DM rate hides a 47% "contact us here: link" rate, and airline problems (delays, lost bags) cannot be answered from tweet text. SpotifyCares has 26,966 threads, 78% English, 31% DM redirects, and ~37% of first replies are substantive (troubleshooting steps, help-centre links, availability answers) about one product with diverse resolvable issues. That is the best ratio of "answerable from public knowledge" in the dataset.

**What "good" means here.** Auto-handle the how-to / troubleshooting / content-availability / feedback tweets with a reply that is grounded in what the brand actually says and sounds like the brand. Never auto-handle anything that needs account access (billing, login), carries a legal/churn threat, contains PII, or is a repeat contact. The dangerous error is an *unsafe auto*: sending a bot reply to someone who needed a human. The tolerable error is over-escalating. So the metrics I care about, in order, are unsafe-auto rate, escalation recall, then reply quality on the auto slice, then intent accuracy.

**What I chose not to build.** Multi-turn threads (the agent sees only the first customer tweet); embeddings or a vector DB (TF-IDF was enough to test the idea); any fine-tuning; a real DM/handoff integration; non-English handling (routed to `other` and escalated); RAG over support.spotify.com (the help-centre links in exemplars are used as-is, never fetched); and an LLM inside the escalation policy — the policy is rules only, on purpose (section 11).
...
I started from a longer proposal and merged/split on 150 random corpus messages. `offline_download` became `downloads_library` because "my whole library disappeared" is as common as "downloads vanished" and gets the same fix path. "Add this song" went into `content_missing`, not feedback, because the reply is the same ("we'll pass it to the label / not licensed in your region"). Praise stays in `feature_feedback` because it gets a real reply, whereas `other` means no reply is needed. I rejected a `usage_question` (how-to) intent — how-tos are almost always about one of the areas above and the 3b model would split hairs — and rejected splitting billing into payment vs plan management because the first reply is nearly always "DM us your email" either way, so there is no routing value.
...
- **The golden set was labelled by the same process that designed the taxonomy.** The intent definitions, the edge rules, and the labels all came from the same head (and the same assistants). A different labeller with a different sense of "is a TV app playback or device_connect?" would move accuracy by several points. The κ on the 60-row overlap is high partly because both passes followed the same written rules.
- **Prompt changes were selected on the golden set.** The classifier prompt went through three iterations on a separate 150-row sample, but the policy thresholds and the drafter's post-processing were adjusted after looking at golden-set output. There is no held-out set. Treat the intent number as optimistic.
- **The "safe" number is bought by escalating most things.** `{{esc_agent_unsafe}}` unsafe autos looks good next to 81 for never-escalate, but the agent escalates `{{esc_agent_share}}` of traffic against a true rate of 37%. `always_escalate` has zero unsafe autos and no one would call that a good agent. The honest comparison is unsafe-auto rate *at the agent's auto rate*, and on that axis the agent is only modestly better than the keyword rule.
- **The judge is a 3b model that barely discriminates.** Means of 3.0–3.2 with 60%+ ties tell you almost nothing about which system writes better replies. Only the judge-vs-human block is real evidence, and it is 120 items from one human (me).
- **Month skew.** 94% of the data is October–November 2017. The golden set is stratified by month to *over*-represent the tails, which makes it not representative of traffic; whichever way you cut it, "iOS 11 / iPhone X" era problems dominate.
- **Retrieval exemplars come from the same corpus as the reference replies.** The TF-IDF index covers all 28k threads (the golden row's own thread is excluded, but a near-duplicate tweet from the same week is not). A "grounded" draft that copies a near-duplicate's reply is scored as grounded and helpful — which is fine for a support bot, but it inflates the gap over a system that would see genuinely new questions.
- **The reference "brand reply" is often itself bad.** Real SpotifyCares replies asked for a DM on things that need no account (`47` duplicate song in Discover Weekly, `218` add-a-song, `9` a joke tweet), answered a hacked-account report with a public link (`207`, `208`), and once read as sarcastic (`12`). The judge treats the reference as ground truth for grounding, so a draft that improves on a bad reference can be penalised.
- **Single-turn only.** Every number is on the first customer tweet in isolation. F1 above shows at least 9/220 rows cannot be answered from that input at all, and the real product would have the thread.
